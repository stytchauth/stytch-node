...
exports.RBACCacheManager = void 0;
...
class RBACCacheManager {
  constructor(fetchConfig) {
    this.rbac = new _rbac.RBAC(fetchConfig);
  }
  fresh() {
...
exports.RBACCacheManager = RBACCacheManager;
function performAuthorizationCheck({
  ...
  const hasPermission = policy.roles.filter(role => subjectRoles.includes(role.role_id)).flatMap(role => role.permissions).some(permission => {
    const hasMatchingAction = permission.actions.includes(authzRequest.action) || permission.actions.includes("*");
    const hasMatchingResource = authzRequest.resource_id === permission.resource_id;
