...
export class SAML {
  ...
  deleteEncryptionPrivateKey(
    data: B2BSSOSAMLDeleteEncryptionPrivateKeyRequest,
    options?: B2BSSOSAMLDeleteEncryptionPrivateKeyRequestOptions
  ): Promise<B2BSSOSAMLDeleteEncryptionPrivateKeyResponse> {
    const headers: Record<string, string> = {};
    if (options?.authorization) {
      addAuthorizationHeaders(headers, options.authorization);
    }
    return request<B2BSSOSAMLDeleteEncryptionPrivateKeyResponse>(
      this.fetchConfig,
      {
        method: "DELETE",
