...
async function generateAccessToken(keyID: string, nowEpoch: number, privateKey: KeyLike, issuer: string):Promise<string> {
...
describe("idp.introspectTokenLocal", () => {
  ...
  beforeEach(async () => {
    ...
    accessToken = await generateAccessToken(keyID, nowEpoch, privateKey, `stytch.com/${PROJECT_ID}`);
  });
  ...
  it("success with base URL issuer", async () => {
    const baseURLIssuerAccessToken = await generateAccessToken(keyID, nowEpoch, privateKey, MOCK_FETCH_CONFIG.baseURL);
